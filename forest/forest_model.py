...
class ForestModel(object):
    ...
    @staticmethod
    def __concat_xpath(env_xpath, append=None, append_path="", append_convert=""):
        if append is not None and len(append) > 0:
            append_path, append_convert = ForestModel.__match_xpath(append)
            append_path = "" if append_path is None else append_path
            append_convert = "" if append_convert is None else append_convert

        env_xpath[0] = "" if env_xpath[0] is None else env_xpath[0]
        env_xpath[1] = "" if env_xpath[1] is None else env_xpath[1]
        if env_xpath[1] is not None and len(env_xpath[1]) > 0:
            append_path = ""
        return [env_xpath[0] + append_path, env_xpath[1] + append_convert]
    ...
    def __normalize_source(self, k, node, level, env):
        ...
        if type(node) is str:
            ...
            if path is not None or convert is not None:
                ...
                if not path.startswith(self.__source_prefix):
                    ...
                    # TODO: this self.__xpath is a default env key, can be set in the future
                    full_path = "{%s}%s" % tuple(self.__concat_xpath(env[k] if k in env else env[self.__xpath],
                                                                     append_path=path, append_convert=convert))
                else:
                    full_path = node

                # if key of full_path is a simplified format, add the default "__xpath"
                node = full_path if k in env else {self.__xpath: full_path}
        ...
        # when the node is dict
            ...
            for k, v in node.items():
                if k in env:
                    # this just fill the current path
                    node[k] = self.__normalize_source(k, v, level, dict(env))
                    # the "__" prefix value will effect the following base value
                    # the env is generated by v
                    env_c[k] = list(self.__match_xpath(node[k]))
            need_clean = False
            # then process the other items
            for k, v in node.items():
                if not k in env:
                    if not k.startswith(self.__private_prefix):
                        need_clean = True
                    node[k] = self.__normalize_source(k, v, level, dict(env_c))
            # clean the items with "__" prefix
            if need_clean:
                node = {k: v for k, v in node.items() if not k in env_c}
        ...
        for k, meta_sources in self.grouped_depend_sources.items():
            ...
            for meta_source in meta_sources:
                ...
                for idx, _ in enumerate(converts):
                    ...
                    while not Converter.equal_method(next_convert,
                                                     converts[idx + 1] if idx + 1 < len(converts) else None):
